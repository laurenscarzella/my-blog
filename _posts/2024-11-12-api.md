...
**3. Clean the Data:**
Read in the messy dataset and filter for relevant columns such as title, popularity, genre_ids, etc. Convert the genre IDs into genre names by making a separate API request using the "genre/movie/list" endpoint and mapping each ID to their corresponding name. If a movie has multiple genres, save the first genre as the primary genre. Drop any unnecessary columns such as genre_ids and genre_names and save the cleaned data to a new CSV file.
